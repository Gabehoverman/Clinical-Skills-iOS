//
//  DetailOverviewComponentViewController.swift
//  Clinical Skills
//
...
class DetailOverviewComponentViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        if self.system != nil {
            ...
            self.remoteConnectionManager = RemoteConnectionManager(delegate: self as! RemoteConnectionManagerDelegate)
        ...
        if self.fetchedResultsController != nil {
            if let managedComponent = self.fetchedResultsController!.object(at: indexPath) as? ComponentManagedObject {
                if self.tabBarController?.selectedIndex == StoryboardIdentifiers.tab.clinicalSkills {
                    self.performSegue(withIdentifier: seg!, sender: managedComponent) //Orginially toComponentFile
                } else {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == seg {
            if let destination = segue.destination as? DetailOverviewViewController {
        ...
        if segue.identifier == seg {
            if let destination = segue.destination as? SpecialTestOverviewTableViewController {
                if let managedComponent = sender as? ComponentManagedObject {
                    destination.component = Component(managedObject: managedComponent)
                }
            }
...
extension DetailOverviewComponentViewController : RemoteConnectionManagerDelegate {
...
extension DetailOverviewComponentViewController : DatastoreManagerDelegate {
...
extension DetailOverviewComponentViewController : UISearchBarDelegate {
